...
const CompletedOrder: FunctionComponent<CompletedOrderProps> = ( {cart} ) => {
  ...
  function backHomeButtonHandler() {
    localStorage.clear();
    router.push('/');
  }
    ...
    <div className={stylesCompletedOrder.wrapper}>
      <div className={`borderRadius ${stylesCompletedOrder.completedOrder}`}>
        ...
        <div className={stylesCompletedOrder.orderContainer}>
          ...
          <div className={isExpand ? `${stylesCompletedOrder.totalPriceWrapper} ${stylesCompletedOrder.alignItemsEnd}` : stylesCompletedOrder.totalPriceWrapper}>
            <div>
              <p className={`${typography.baseText} ${typography.uppercaseText} ${stylesCheckout.paddingBottom8px}`}>
                grand total
              </p>
