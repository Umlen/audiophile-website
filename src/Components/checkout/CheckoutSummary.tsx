...
const CheckoutSummary: FunctionComponent<CheckoutSummaryProps> = ({
  cart,
  isCashPay,
}) => {
  ...
  return (
    <section
      className={`borderRadius ${stylesCheckout.whiteSection} ${stylesCheckout.summarySection}`}
    >
      <h2
        className={`${typography.bold18px} ${stylesCheckout.paddingBottom8px}`}
      >
        summary
      </h2>
      {cart.map((product) => (
      ...
      <div>
        <div
          className={`${stylesCheckout.flexContainer} ${stylesCheckout.paddingBottom8px}`}
        >
      ...
      <div className={stylesCheckout.flexContainer}>
        ...
        <p className={`${typography.bold18px} ${typography.highlightText}`}>
          $ {grandTotal}
        </p>
      </div>
      <OrangeButtonWide
